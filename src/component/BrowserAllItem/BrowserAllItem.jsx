...
export default function BrowserAllItem({ searchQuery = "" }) {
  ...
  const fetchItems = async (currentStatus, isBackgroundRefresh = false) => {
    try {
      ...
      // Adjusted data extraction based on your console log
  ...
  const getDisplayStatus = (item) => {
    if (item.find === true || item.status === "CLAIMED" || item.type === "claim") return "CLAIM";
  ...
  const filteredData = items.filter((item) => {
    ...
    const matchesSearch = !normalizedSearch || [
        item.itemTitle, item.itemName, item.title, item.description, item.location,
    ].some((value) => String(value || "").toLowerCase().includes(normalizedSearch));

    if (!matchesSearch) return false;    

    // FIX: Removed the condition that was returning false for "All Items" 
    // This allows items where find === true to show up in the "All Items" list.
  ...
  const getItemName = (item) => {
    return item.itemTitle || item.itemName || item.title || "Unknown Item";
  };
                ...
                return (
                  ...
                  >
                    <div className="bg-blue-50 h-40 flex items-center justify-center relative">
                      <span className={`absolute top-3 left-3 z-10 text-[10px] px-3 py-1 rounded-full font-bold ${statusColor[displayStatus] || "bg-gray-200"}`}>
                        {displayStatus}
                      </span>
                      {item.image ? (
                        <img src={item.image} alt="item" className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                      ) : (
