...
export default function BrowserAllItem() {
  ...
  // ✅ Added isBackgroundRefresh to prevent loading flicker
  const fetchItems = async (isBackgroundRefresh = false) => {
    try {
      ...
      // ✅ CRITICAL FIX: Update the open modal with fresh DB data
      setSelectedItem((prev) => {
        if (!prev) return null;
        const updatedItem = finalData.find((i) => i._id === prev._id);
  ...
  const discoverFields = (data) => {
    const statusWords = [
      "lost", "found", "claimed", "claim", "posted", "pending", "active", "inactive", "resolved", "rejected",
    ];
    ...
    const categoryWords = [
      "electronic", "electronics", "wallet", "wallets", "card", "cards", "id card", "id cards", "idcard",
      "bag", "bags", "other", "document", "documents", "key", "keys", "phone", "mobile", "laptop",
      "book", "books", "clothing", "accessory", "watch", "bottle", "umbrella", "jewelry", "vehicle",
    ];
    ...
    for (const key of Object.keys(data[0])) {
      ...
      const values = data
        .map((i) => String(i[key] || "").toLowerCase().trim())
        .filter(Boolean);
    ...
    if (catField) {
      ...
      for (const item of data) {
        for (const val of Object.values(item)) {
          ...
          fallbackCats.add(s.charAt(0).toUpperCase() + s.slice(1).toLowerCase());
        }
      }
    ...
    for (const item of data) {
      for (const key of Object.keys(item)) {
        ...
        const val = item[key];
        if (val && typeof val === "string" && /^\d{4}-\d{2}-\d{2}/.test(val) && !isNaN(new Date(val).getTime())) {
  ...
  const filteredData = items.filter((item) => {
    ...
    if (!matchesStatus) {
      ...
      if (discoveredFields.current.status) {
        rawStatus = String(item[discoveredFields.current.status] || "").toLowerCase().trim();
      } else {
        rawStatus = String(item.itemStatus || item.status || item.type || item.itemType || "").toLowerCase().trim();
      }
    ...
    if (!matchesCategory) {
      ...
      if (discoveredFields.current.category) {
        const raw = String(item[discoveredFields.current.category] || "").toLowerCase().trim();
    ...
    if (dateFilter !== "All Dates") {
      ...
      if (discoveredFields.current.date) {
        ...
        for (const v of Object.values(item)) {
          if (v && typeof v === "string" && v.length > 10 && !isNaN(new Date(v).getTime())) {
            dateStr = v;
            break;
          }
        }
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-100 p-4 md:p-8 relative overflow-hidden">
        <div className={`transition-all duration-300 ${selectedItem ? "blur-sm scale-[0.99]" : ""}`}>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-3 sm:p-4 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
              <span className="text-gray-500 text-xs sm:text-sm font-medium whitespace-nowrap">Filter:</span>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {loading ? (
              ...
              filteredData.map((item) => {
                const displayStatus = (
                  item.itemStatus || item.status || item.type || item.itemType || "POSTED"
                ).toUpperCase();
                  ...
                  >
                    ...
                    <div className="p-5">
                      <h3 className="font-bold text-lg text-gray-800 truncate">
                        {item.itemName || item.itemTitle || item.title || "Untitled Item"}
                      </h3>
                      ...
                      <div className="flex flex-col gap-1 text-xs text-gray-400 mt-4">
                        ...
                        {item.claimedBy && (
                          <span className="text-green-600 font-medium mt-1">
                            ✅ Claimed by: {item.claimedBy.name || item.claimedBy.email}
                          </span>
                        )}
                      </div>
                    </div>
              ...
              <div className="col-span-full py-20 text-center bg-white rounded-3xl border-2 border-dashed">
                <p className="text-gray-500">No items found for "{statusFilter}"</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-3 sm:px-4">
            ...
            <div className="relative z-50 w-full flex items-center justify-center">
              <ItemDetails
                ...
                // ✅ Pass as background refresh so skeletons don't flicker
