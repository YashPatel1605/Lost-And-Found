...
export default function LostSomething({ items = [], loading = false }) {
	...
	const stats = useMemo(() => {
		...
		const recoveredItems = items.filter(
			(item) => item.find === true || item.status === 'CLAIMED' || item.itemStatus === 'CLAIMED',
		).length

		const uniqueUsers = new Set(
			items
				.map((item) => item.userId || item.createdBy || item.user?._id || item.user?.id)
				.filter(Boolean),
		)
		...
		<div className="min-h-screen bg-linear-to-r from-[#0b1d3a] via-[#0f2a4d] to-[#0b1d3a] text-white flex items-center justify-center px-4">
			<div className="max-w-7xl w-full text-center">
				...
				<p className="text-gray-300 mt-6 max-w-2xl mx-auto text-sm sm:text-base md:text-lg">
					The smartest lost & found platform for your campus. Report, search, and recover items with
