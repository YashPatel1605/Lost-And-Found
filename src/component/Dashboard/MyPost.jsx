...
export default function MyPosts() {
  ...
  const handleMakeAsClaim = async (id) => {
    ...
    if (
      currentItem?.find ||
      currentItem?.type === "claim" ||
      claimingItemsRef.current.has(id)
    )
      return;
    ...
    try {
      ...
      setItems((prevItems) => {
        const updatedItems = prevItems.map((item) =>
          item._id === id
            ? { ...item, find: true, type: "claim", status: "CLAIMED" }
            : item
        );
        myPostsStore.items = updatedItems;
        return updatedItems;
  ...
  const getDisplayStatus = (item) => {
    ...
    const baseStatus = (item.status || item.type || "PENDING").toUpperCase();
    ...
    return { label: baseStatus, className: "bg-yellow-100 text-yellow-700" };
  };
    ...
    <>
        ...
        <div className="flex-1 p-4 md:p-8">
              ...
              <tbody>
                {Array.isArray(items) && items.length > 0 ? (
                  items.map((item) => {
                    ...
                    const isClaimed =
                      item.find === true || item.type === "claim";
                      ...
                      <tr key={item._id} className="border-t">
                        ...
                        <td className="p-4 flex justify-center gap-2">
                          <button
                            ...
                            disabled={
                              isClaimed || claimingItemId === item._id
                            }
          ...
          <div className="md:hidden space-y-4">
            {Array.isArray(items) && items.length > 0 ? (
              items.map((item) => {
                const displayStatus = getDisplayStatus(item);
                const isClaimed = item.find === true || item.type === "claim";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${displayStatus.className}`}
                      >
                        {displayStatus.label}
                      </span>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      <button
                        onClick={() => handleMakeAsClaim(item._id)}
                        disabled={isClaimed || claimingItemId === item._id}
                      ...
                      <button
                        onClick={() => handleEdit(item._id)}
                        className="flex-1 bg-yellow-100 text-yellow-700 py-2 rounded-md text-sm"
                      >
                        Edit
                      </button>
                      ...
                      <button
                        onClick={() => openDeleteModal(item._id)}
                        className="flex-1 bg-red-100 text-red-600 py-2 rounded-md text-sm"
                      >
                        Delete
                      </button>
                    </div>
      ...
      {deleteItemId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-[fadeIn_.2s_ease]">
            <div className="p-6">
